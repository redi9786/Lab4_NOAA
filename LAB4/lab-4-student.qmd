...
```{r}
#| label: load-data
NOAA_BUOY <- read_table("buoy_2024.txt")
...
NOAA_BUOY <- read_table("buoy_2024.txt", comment = "#")
head(NOAA_BUOY)
```
...
```{r}
#| label: clean-names
clean_NOAA_BUOY <- NOAA_BUOY |>
  clean_names()
  
  head(clean_NOAA_BUOY)
```
...
write_csv(clean_NOAA_BUOY, "NOAA_BUOY_update.csv")
```
...
BUOY_NAs <- read_csv("NOAA_BUOY_update.csv",
...
ggplot(buoy_final, aes(x = yy, y = atmp, color = factor(mm))) +
geom_point(alpha = 0.6) +
labs(title = "Monterey Air Temperature by Month",
x = "Year",
y = "Air Temp (C)",
color = "Month") +
theme_minimal()
...
> What is the biomass of kelp aand sea urchins between MPAs and Refernce sites(non-MPAs)?
https://piscoweb.org/data-access
